...
class ApiService {
  ...
  /// 400: 에러
  static Future<List<QuestionModel>> getQuestionsListQuestions() async {
    int userId = await getUserId();
  ...
  // Tab3; 달력 페이지
  // 날짜별 대표 색상 불러오기
  static Future<List<DailyColorModel>> getDailyColor() async {
    int userId = await getUserId();
  ...
  static Future<int> getConsecutiveDates() async {
    int userId = await getUserId();
